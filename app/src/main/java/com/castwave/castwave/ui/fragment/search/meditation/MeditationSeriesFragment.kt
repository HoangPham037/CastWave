...
@AndroidEntryPoint
class MeditationSeriesFragment : BaseFragment<FragmentMeditationSeriesBinding>() {
    ...
    private fun initAction() {
        ...
        binding.ivTimer.setOnClickListener {
            openScreenWithContainer(requireContext(), ChooseTimerFragment::class.java, null)
        }
    }

    private fun initAdapter() {
        binding.rcvCategory.adapter = categoryAudiobookAdapter
        binding.rcvMeditation.adapter = adapterMeditation
        adapterMeditation.setEvent {
            openScreenWithContainer(requireContext(), MeditationCategoryFragment::class.java, null)
        }
        categoryAudiobookAdapter.items = listCategoryAudioBook()
    ...
    private fun initView() {
        binding.mainAppbar.addOnOffsetChangedListener { appBarLayout, verticalOffset ->
            ...
            binding.tvTitle.setVisibility(!boolean)
            binding.tvContent.setVisibility(!boolean)
    ...
    override fun getLayoutBinding(): FragmentMeditationSeriesBinding =
        FragmentMeditationSeriesBinding.inflate(layoutInflater)
}